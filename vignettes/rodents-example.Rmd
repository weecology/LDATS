...
changepoint_models <- TS_on_LDA(LDA_models = selected_lda_model, 
                                ...
                                weights = document_weights(rodents$document_term_table),
                                                           control = TS_controls_list(nit = 100))
...
```{r plot cpt, fig.width=6, fig.height=6}
...
```{r plot LDA_TS results, fig.height = 6, fig.width = 6, fig.show = 'hold'}

plot(lda_ts_results$`Selected LDA model`)

plot(lda_ts_results$`Selected TS model`)
```