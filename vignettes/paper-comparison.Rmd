...
```{r cleanup dates, include = F, eval = FALSE}
rm(moondat)
rm(new_cov_table)
```
...
png(file.path(paper_filepath, "a.png"), width = 800, height = 400)
...
```{r}
knitr::include_graphics(file.path(paper_filepath, "a.png"))
```
...
compute_cpt <- function(cpt_results, ntopics)
{
  ...
  return(best_cpt_model)
}

ldats_paper_cpt <- compute_cpt(ldats_paper_results, 
                               ntopics = ldats_lda_selected[[1]]@k)

ldats_paper_cpt_dates <- ldats_paper_cpt$saved[,1,] %>%
  t() %>%
  as.data.frame() %>%
  reshape::melt() %>%
...
```{r run paper LDA and paper cpt, eval = TRUE}
#### Run changepoint ####
...
paper_paper_cpt <- compute_cpt(paper_paper_results, 
                               ntopics = ldamodel@k)

paper_paper_cpt_dates <- paper_paper_cpt$saved[,1,] %>%
  t() %>%
  reshape::melt() %>%
  dplyr::left_join(paper_covariates, by = c("value" = "index"))
```
