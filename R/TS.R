...
#'   TSmod <- TS(data, gamma ~ 1, nchangepoints = 1, weights, "newmoon")
#' }
#'
#' @export
#'
TS <- function(data, formula, nchangepoints, weights, timename = "time",
               control = TS_controls_list()){
  check_TS_inputs(data, formula, nchangepoints, weights, timename, control)
  set.seed(control$seed)
  data <- data[order(data[,timename]), ]
  rho_dist <- est_changepoints(data, formula, nchangepoints, weights, 
                               timename, control)
  eta_dist <- est_regressors(rho_dist, data, formula, weights, 
                             timename, control)
  summarize_TS(data, formula, weights, timename, control, rho_dist, eta_dist)
}
...
#' @export
#'
check_TS_inputs <- function(data, formula, nchangepoints, weights, timename,
                            control){
...
#' @export
#'
summarize_TS <- function(data, formula, weights, timename, control, rho_dist, 
                         eta_dist){
  ...
  out <- list(data = data, formula = formula, nchangepoints = nchangepoints,
              weights = weights,
              timename = timename, control = control, lls = lls, rhos = rhos,
...
#' @export
#'
est_regressors <- function(rho_dist, data, formula, weights, timename, 
                           control){
  check_formula(data, formula)  
  check_weights(weights)
  check_control(control, "TS_controls")
  ...
  for(i in 1:nr){
    ...
    mods <- multinom_TS(data, formula, cpts, weights, timename, control)
    ndraws <- freq_r[i]
    colindex1 <- 1
...
#' @export
#'
est_changepoints <- function(data, formula, nchangepoints, weights, timename,
                             control){
  check_TS_inputs(data, formula, nchangepoints, weights, timename, control)
  if (nchangepoints == 0){
    return(NULL)
...
#'
#' @return List of class \code{TS_controls}, with named elements corresponding
#'   to the arguments.
#'
#' @export
#'
