...
# functions that probably should get copied over




  
cp_hists_file <- function(dd = NULL, cpms = NULL, ntopics = NULL, nn = NULL){
        ...
        jpeg(paste("cp_", nn, ".jpeg", sep = ""), height = 10, width = 6, 
             unit = "in", res = 200)
        par(mfrow = c(5,1))
        annual_hist(cpms[[1]], year_continuous)
        text(1973, 1.1e5, cex = 1.25, adj = -1, xpd = T,
         paste("AIC = ", 
         round(mean(cpms[[1]]$saved_lls * -2) + 2 * (3 * (ntopics - 1) * (1 + 1) + (1)), 2),
                   sep = ""))
        mtext(side = 3, nn, line = 1)
        annual_hist(cpms[[2]], year_continuous)
        text(1973, 1.1e5, cex = 1.25, adj = -1, xpd = T,
         paste("AIC = ", 
         round(mean(cpms[[2]]$saved_lls * -2) + 2 * (3 * (ntopics - 1) * (2 + 1) + (2)), 2),
                   sep = ""))
        annual_hist(cpms[[3]], year_continuous)
        text(1973, 1.1e5, cex = 1.25, adj = -1, xpd = T,
         paste("AIC = ", 
         round(mean(cpms[[3]]$saved_lls * -2) + 2 * (3 * (ntopics - 1) * (3 + 1) + (3)), 2),
                   sep = ""))
        annual_hist(cpms[[4]], year_continuous)
        text(1973, 1.1e5, cex = 1.25, adj = -1, xpd = T,
         paste("AIC = ", 
         round(mean(cpms[[4]]$saved_lls * -2) + 2 * (3 * (ntopics - 1) * (4 + 1) + (4)), 2),
                   sep = ""))
        annual_hist(cpms[[5]], year_continuous)
        text(1973, 1.1e5, cex = 1.25, adj = -1, xpd = T,
         paste("AIC = ", 
         round(mean(cpms[[5]]$saved_lls * -2) + 2 * (3 * (ntopics - 1) * (5 + 1) + (5)), 2),
                   sep = ""))
        dev.off()
}
